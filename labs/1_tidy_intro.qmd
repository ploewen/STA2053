...
dm |>
  select(Year:Female) |>
  group_by(Year) |>
  summarise(Highest_mortality_age = Age[which.max(Female)])
```
...
dm |>
  select(Year:Total) |>
  group_by(Age) |>
  summarise(across(Female:Total, sd))
```
...
```{r}
pcan <- read_table("https://www.prdh.umontreal.ca/BDLC/data/ont/Population.txt", 
                   skip = 1, col_types = "dcddd")
```

```{r}
joined <- dm |>
  left_join(pcan, by = join_by(Year, Age))

fwamr <- joined |>
  group_by(Year) |>
  summarise(Female = weighted.mean(Female.x, Female.y))
mwamr <- joined |>
  group_by(Year) |>
  summarise(Male = weighted.mean(Male.x, Male.y))

left_join(fwamr, mwamr, by = join_by(Year)) |>
  pivot_longer(cols = Female:Male,
               names_to = "Sex", values_to = "Weighted_Average_Mortality") |>
  ggplot(aes(x = Year, y = Weighted_Average_Mortality, color = Sex)) +
  geom_line() +
  scale_color_brewer(palette = "Set1") + 
  theme_bw(base_size = 14) +
  labs(y = "Weighted Average Mortality Rate")
```

We notice that the population weighted average mortality rate is higher than the non-weighted average mortality rate, with a steeper decrease until the late 20th century, followed by a short period of increased mortality rates. 